<template>
  <form :class="{'was-validated': isSubmited}" novalidate @submit.prevent="createProducts">
    <div class="row justify-content-center">
        ...
        <div v-for="(product,index) in productsToCreate" :key="index">
          <div class="card mb-3">
            ...
            <div class="card-body">
              ...
              <div class="form-row mb-3">
                <base-image-uploader @uploadSuccess="uploadSuccessHandler(index, $event)"/>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="col-8">
        <button :disabled="submitDisabled" class="btn btn-primary" type="submit">Submit form</button>
      </div>
    </div>
  </form>
</template>
...
export default {
  ...
  computed: {
    submitDisabled() {
      return this.productsToCreate.length === 0
    }
  },
  methods: {
    getEmptyProduct() {
